        ...
        if (!Reset) begin
            ...
            if (Reverse) begin
                ...
                if ((Data >= 0) && (Data < 2**(Size-1)-1))
                    Data <= Data + 1'b1;
                else if ((Data > 2**(Size-1)+1) && (Data < 2**Size))
                    Data <= Data - 1'b1;
                else if (Data == 2**(Size-1)+1)
                    Data <= 0;
            end
            else if (Down) begin
                if ((Data > 0) && (Data < 2**(Size-1)))
                    Data <= Data - 1'b1;
                else if ((Data > 2**(Size-1)) && (Data < 2**Size-1))
                    Data <= Data + 1'b1;
                else if (Data == 0)
                    Data <= 2**(Size-1)+1;
            end
...
module Data2Segments #(
    ...
    always_comb begin
        if (Signed == 0)
            ...
            BCD = {(Data[Size-1] == 1'b0) ? 4'hf : 4'ha,
